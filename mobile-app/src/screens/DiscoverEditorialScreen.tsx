...
function DiscoverHeader({
  ...
  return (
    <View style={{ backgroundColor: theme.bg, paddingTop: 60, paddingHorizontal: 16, paddingBottom: 14, borderBottomWidth: 1, borderBottomColor: theme.divider }}>
      {/* Home-style wordmark row — carries over Home's centered "wugi" +
          WeatherBadge. Search no longer sits in a persistent pressable bar;
          a top-right icon expands the search row below on tap. Collapses
          when search is active, same as the old title row did. */}
      {!searchActive && (
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
          <View style={{ width: 76 }}/>
          <View style={{ flex: 1, alignItems: 'center' }}>
            <Text style={{ color: theme.accent, fontSize: 28, fontFamily: FONTS.display, letterSpacing: -1.1, lineHeight: 32 }}>wugi</Text>
          </View>
          <View style={{ width: 76, flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', gap: 14 }}>
            <WeatherBadge theme={theme}/>
            <TouchableOpacity onPress={onActivate} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
              <SearchIcon color={theme.text}/>
            </TouchableOpacity>
            <TouchableOpacity onPress={onMapTap} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
              <MapIcon color={theme.text}/>
            </TouchableOpacity>
          </View>
        </View>
      )}
      {/* Search row — drops down in place of the wordmark row once the
          search icon is tapped. Same input behavior as before. */}
      {searchActive && (
...
function EditorialBody({ shelves, loading, theme, onCard, venueNameById, galleryFallback, deals, onDealPress, onGalleryPress }: {
  ...
  // The single-photographer "Photographer Feature" shelf is replaced by the
  // shared RecentGalleries shelf below — filter it out of the generic
  // editorial shelf list so it doesn't render twice.
