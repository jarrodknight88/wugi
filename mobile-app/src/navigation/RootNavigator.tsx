...
import { ClaimTransferScreen }   from '../features/ticketing/ClaimTransferScreen';
...
function Navigator({ onNotificationNavigate }: { onNotificationNavigate?: (fn: (data: Record<string, string>) => void) => void }) {
  ...
  // ── Ticket-transfer claim deep link ──────────────────────────────────
  // Handles wugi://tickets/claim/{token} (custom scheme) and
  // https://wugi.us/tickets/claim/{token} (universal link, already
  // registered via app.json's associatedDomains). Both are generated by
  // initiateTransfer and normally land recipients on the web claim page
  // (web/app/tickets/claim/[token]) — this lets the app claim the ticket
  // in place when Wugi is already installed.
  const lastClaimTokenRef = useRef<string | null>(null);
  const handleClaimUrl = useCallback((url: string | null) => {
    const token = parseClaimToken(url);
    if (!token || token === lastClaimTokenRef.current) return;
    lastClaimTokenRef.current = token;
    // A deep link is explicit user intent — skip splash/intro straight to the claim screen.
    routedRef.current = true;
    splashDoneRef.current = true;
    setAppPhase('main');
    push({ screen: 'claimTransfer', token });
  }, []);

  useEffect(() => {
    Linking.getInitialURL().then(handleClaimUrl).catch(() => {});
  ...
  const renderStackScreen = () => {
    ...
    if (current.screen === 'photo')   return <PhotoViewer    photos={current.photos} initialIndex={current.initialIndex} galleryTitle={current.galleryTitle} venue={current.venue} date={current.date} onBack={pop} onVenuePress={current.venueId ? () => navigateToVenueById(current.venueId) : undefined} likedPhotoIds={favorites.filter(f => f.type === 'photo').map(f => f.id)} onPhotoLikeChange={onPhotoLikeChange} uid={uid} onRequireAuth={() => push({ screen: 'auth', intent: 'unlock' })} theme={theme}/>;
    ...
    if (current.screen === 'claimTransfer') return (
      <ClaimTransferScreen
        token={current.token}
        userId={user?.uid ?? null}
        userEmail={user?.email ?? ''}
        userName={user?.displayName ?? ''}
        onClaimed={(orderId, isGuest) => {
