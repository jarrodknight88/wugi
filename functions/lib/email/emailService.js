...
const btn = (href, label) => `<a href="${href}" style="display:inline-block;background:${GREEN};color:#fff;font-weight:700;font-size:15px;padding:14px 28px;border-radius:${RADIUS};text-decoration:none;margin-top:20px">${label}</a>`;
const divider = `<hr style="border:none;border-top:1px solid #f0f0f0;margin:24px 0">`;
const row = (label, value) => `<tr><td style="color:#888;font-size:13px;padding:6px 0">${label}</td>
...
async function sendTransferNotification(data) {
    const resend = getResend();
    const html = wrap(`
...
async function sendReclaimEmail(data) {
    const resend = getResend();
    const reclaim = `https://wugi.us/tickets/${data.orderId}`;
    const html = wrap(`
