...
exports.onTableColorChange = exports.passWebService = void 0;
// ─────────────────────────────────────────────────────────────────────
...
async function validateAuthToken(serial, token) {
    try {
...
exports.passWebService = functions.https.onRequest(async (req, res) => {
    ...
    if (method === 'GET' && parts[1] === 'passes' && parts.length === 4) {
        ...
        if (passType !== PASS_TYPE) {
            res.status(400).send();
            return;
        }
        const authHeader = req.headers.authorization || '';
        const token = authHeader.replace('ApplePass ', '');
        if (!await validateAuthToken(serial, token)) {
            res.status(401).send();
            return;
        }
        try {
    ...
    if (method === 'GET' && parts[1] === 'devices' && parts[3] === 'registrations' && parts.length === 5) {
        ...
        try {
            ...
            for (const reg of regsSnap.docs) {
                ...
                if (!passDoc.exists)
                    continue;
                const lastUpdated = passDoc.data()?.lastUpdated;
...
exports.onTableColorChange = functions.firestore
    .document('venues/{venueId}/tableColors/{tableId}')
    .onUpdate(async (change, context) => {
    const before = change.before.data();
    const after = change.after.data();
    // Only proceed if color actually changed
    if (before.color === after.color && before.label === after.label)
        return;
    ...
    try {
        ...
        for (const orderDoc of ordersSnap.docs) {
            ...
            const order = orderDoc.data();
            pushPromises.push((0, generatePass_1.buildPassBuffer)({
                orderId,
                eventTitle: order.eventTitle || '',
                venueName: order.venueName || '',
                eventDate: order.eventDate || '',
...
async function pushApnsToWalletDevices(orderId) {
...
//# sourceMappingURL=passWebService.js.map