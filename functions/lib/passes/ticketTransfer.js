...
exports.cancelTransfer = exports.claimTransfer = exports.initiateTransfer = void 0;
// ─────────────────────────────────────────────────────────────────────
// Wugi — ticketTransfer
...
exports.initiateTransfer = functions.runWith({ secrets: ['RESEND_API_KEY'] }).https.onRequest(async (req, res) => {
    ...
    if (!orderId || !toEmail) {
        res.status(400).json({ error: 'orderId and toEmail required' });
        return;
    }
    try {
        ...
        if (order.checkedIn) {
            res.status(400).json({ error: 'Cannot transfer a checked-in ticket' });
            return;
        }
        if (order.transferred) {
            res.status(400).json({ error: 'This ticket has already been transferred' });
            return;
        }
...
exports.claimTransfer = functions.https.onRequest(async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
    ...
    if (!token || !claimerEmail) {
        res.status(400).json({ error: 'token and claimerEmail required' });
        return;
    }
    try {
        ...
        if (transferSnap.empty) {
            res.status(404).json({ error: 'Transfer not found or already claimed' });
            return;
        }
        ...
        if (transfer.expiresAt.toDate() < new Date()) {
            ...
            res.status(400).json({ error: 'Transfer link has expired' });
            return;
        }
        // Get original order
...
// ── cancelTransfer ───────────────────────────────────────────────────
exports.cancelTransfer = functions.https.onRequest(async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
    res.set('Access-Control-Allow-Headers', 'Content-Type');
    if (req.method === 'OPTIONS') {
    ...
    if (!transferId || !orderId) {
        res.status(400).json({ error: 'transferId and orderId required' });
        return;
    }
    try {
