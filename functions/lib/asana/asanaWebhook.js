...
function validateSignature(rawBody, receivedHmac, secret) {
...
exports.asanaWebhook = functions.onRequest({
    ...
    const receivedHmac = req.headers['x-hook-hmac-sha256'];
    if (!receivedHmac) {
    ...
    try {
        ...
        for (const event of payload.events) {
            ...
            let story;
            try {
                story = await (0, asanaClient_1.fetchStory)(storyGid, devAgentToken);
            }
            catch (err) {
            ...
            let task;
            try {
                task = await (0, asanaClient_1.fetchTask)(taskGid, devAgentToken);
            }
            catch (err) {
            ...
            let diagnosis;
            try {
                diagnosis = await (0, claudeClient_1.diagnoseWithClaude)(task, commentText, claudeApiKey);
                logger.info('Claude diagnosis complete', { taskGid });
            }
            catch (err) {
                logger.error('Claude API failed', { taskGid, err });
                await (0, asanaClient_1.postTaskComment)(taskGid, '⚠️ @WugiAI encountered an error calling the Claude API. Check Cloud Function logs.', devAgentToken);
                continue;
            }
            // Post diagnosis back to the Asana task as Dev Agent
            const responseComment = `🤖 **@WugiAI Diagnosis**\n\n${diagnosis}`;
            try {
                await (0, asanaClient_1.postTaskComment)(taskGid, responseComment, devAgentToken);
            ...
            try {
                await sendSms(JARROD_PHONE, `🤖 @WugiAI diagnosed "${task.name}". Check Asana.`, twilioSid, twilioToken, twilioFrom);
...
//# sourceMappingURL=asanaWebhook.js.map