  ...
  try {
    switch (event.type) {
      case 'payment_intent.succeeded':
        ...
        if ((event.data.object as Stripe.PaymentIntent).metadata?.type === 'balance_payment') {
          await handleBalancePayment(event.data.object as Stripe.PaymentIntent);
        } else {
...
async function handlePaymentSuccess(paymentIntent: Stripe.PaymentIntent) {
  ...
  if (!venueData) throw new Error(`Venue ${venueId} not found`);
  ...
  const reserveAmount  = calculateReserve(subtotal, reservePercent);
  ...
  const payoutReleaseAt = new Date(
    eventEndsAt.getTime() + reserveHoldHours * 60 * 60 * 1000
  );
...
async function handleBalancePayment(paymentIntent: Stripe.PaymentIntent) {
...
async function handleDisputeCreated(dispute: Stripe.Dispute) {
  ...
  const scanEvidenceAttached = passesSnap.docs.some(
    p => p.data().scanStatus === 'scanned'
  );
...
async function handleDisputeUpdated(dispute: Stripe.Dispute) {
  ...
  if (chargebackSnap.empty) return;
...
function getPaymentMethodType(charge: Stripe.Charge | null): string {
  ...
  if (wallet?.type === 'apple_pay')  return 'apple_pay';
  if (wallet?.type === 'google_pay') return 'google_pay';
  return 'card';
}

function getNextDailyBatchTime(): Date {
  // Next 2am ET
  const now  = new Date();
  const next = new Date();
