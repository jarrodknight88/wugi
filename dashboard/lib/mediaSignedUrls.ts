...
export type SignedMediaAsset = { url: string; thumbUrl: string; type: "image" | "video"; path: string; moderationStatus: ModerationStatus }
...
export async function signMediaAssets(entries: MediaAssetEntry[]): Promise<SignedMediaAsset[]> {
  ...
  const signed = await Promise.all(
    entries.map(async (entry): Promise<SignedMediaAsset | null> => {
      ...
      if (!url) return null
      return { url, thumbUrl: posterUrl || url, type: entry.type, path: entry.path, moderationStatus: entry.moderationStatus }
    })
  )
