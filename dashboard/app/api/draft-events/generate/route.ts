...
export async function POST(req: NextRequest) {
  ...
  if (mode === "series") {
    ...
    prompt = [
      ...
      "Write a clean, generic series title (no emoji, no hashtags, under 80 characters) and a 4-6 sentence 'about' blurb describing the typical weekly vibe, in an upbeat nightlife voice (no emoji, no hashtags, no mention of any specific guest, artist, or one-off theme — this copy must still make sense for next week and every week after).",
    ...
    prompt = [
      ...
      "",
      "Write a clean event title (no emoji, no hashtags, under 80 characters) and a 4-6 sentence 'about' blurb in an upbeat nightlife voice (no emoji, no hashtags).",
      "The about must preserve concrete facts from the caption when present: prices and specials (e.g. $5 margaritas, $150 bottles), times (happy hour windows, door-to-close), DJ/host/lineup names, sponsors, and recurring perks (free entry, free valet, ladies specials).",
      "Never include the venue name in the title — it's already shown elsewhere on the card.",
      "If the title would otherwise end with a known event format (e.g. 'Trivia Night', 'Dinner Party', 'Karaoke'), lead with the format instead: '{Format}: {Theme}', not '{Theme} {Format}'.",
      'Respond with ONLY a JSON object: {"title":"...","about":"..."}',
    ].join("\n")
