...
export const ACCOUNT_TYPES = ["venue", "promoter", "photographer", "dj_artist"] as const
...
export async function POST(req: NextRequest) {
  ...
  if (status === "approved" && !ACCOUNT_TYPES.includes(accountType)) {
    return NextResponse.json(
      { error: "accountType is required when approving (venue | promoter | photographer | dj_artist)" },
      { status: 400 }
    )
  }
